...
-- FIXED: Moved allocations table after placeholder_roles table
...
CREATE POLICY "projects_read" ON projects FOR SELECT USING (
  ...
  OR EXISTS (
    ...
    AND project_members.user_id = (SELECT id FROM users WHERE auth_id = auth.uid())
  )
);
...
-- ============================================================================
-- Tasks: Allow task visibility
-- ============================================================================
CREATE POLICY "tasks_read" ON tasks FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM projects 
    WHERE projects.id = tasks.project_id
    AND (projects.organization_id = get_current_user_org()
      OR EXISTS (
        SELECT 1 FROM project_members 
        WHERE project_members.project_id = projects.id
        AND project_members.user_id = (SELECT id FROM users WHERE auth_id = auth.uid())
...
CREATE POLICY "tasks_update_assigned" ON tasks FOR UPDATE USING (
  ...
  OR EXISTS (
    SELECT 1 FROM projects 
    WHERE projects.id = tasks.project_id
    AND projects.owner_id = (SELECT id FROM users WHERE auth_id = auth.uid())
  )
);

-- ============================================================================
-- Timesheets: Allow user to see own timesheets
-- ============================================================================
CREATE POLICY "timesheets_read" ON timesheets FOR SELECT USING (
  user_id = (SELECT id FROM users WHERE auth_id = auth.uid())
  OR EXISTS (
    SELECT 1 FROM users 
    WHERE users.organization_id = timesheets.organization_id
    AND users.auth_id = auth.uid() 
    AND users.role IN ('admin', 'resource_manager', 'finance')
  )
);

CREATE POLICY "timesheets_update_own" ON timesheets FOR UPDATE USING (
  user_id = (SELECT id FROM users WHERE auth_id = auth.uid())
);

-- ============================================================================
-- Time Entries: Allow user to see own time entries
-- ============================================================================
...
-- ============================================================================
-- Invoices: Allow finance and admin to see invoices
-- ============================================================================
